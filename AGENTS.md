...
- Use the versioned conda `clang-12` executable for MDL's `clang_PATH` bitcode-generation helper on non-macOS platforms. On macOS, do not install `clang-12` alongside the Clang 22 compiler wrappers because their `llvm-tools` requirements conflict; use the active Clang 22 wrapper for `clang_PATH` and keep the package C/C++ compiler wrappers pinned to Clang 22.1.8 so embedded LLVM 12 is not compiled with Clang 12 against current Xcode libc++ headers.
