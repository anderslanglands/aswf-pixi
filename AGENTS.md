...
- Allow current and future `openusd-typhoon` recipes to publish to both test and default labels; historical preview recipes may remain test-only. Keep `extra.allowed_publish_targets = [test-label, default-label]` on new recipes so CI/build-number/publish scripts enforce the intended destinations. Version Typhoon packages as `<OpenUSD major.minor>.<recipe-build-serial>.<short-commit>`, with one repository-wide serial that starts at `1` and increments for each new Typhoon package definition; for example `26.08.10.121e74ef7`. Use dot separators because rattler normalizes `+` and `-` version separators to `.` in rendered package metadata. Build Typhoon with channels ordered as test label, `anderslanglands`, then conda-forge, and with `--channel-priority disabled` because dependencies may be split across test and main labels. Typhoon's upstream branch pixi manifest uses Embree with OpenQMC 0.7.1, and hdEmbree links `OpenQMC::OpenQMC` unconditionally at the current revision; keep `embree 4.4.*` and `openqmc-dev ==0.7.1` in host and run requirements while Embree is enabled. Do not pass the stale upstream-manifest variables `PXR_ENABLE_OPENQMC_SUPPORT` or `PXR_HDEMBREE_ENABLE_OPENQMC`; the current CMake source does not consume them. Keep `openimageio-dev 2.5.*` for Typhoon even though the latest full OpenUSD recipe uses 3.1, because the branch's hdEmbree code uses the OIIO 2.5 TextureSystem/TextureOpt API. Do not add NumPy just because it appears in the upstream development pixi manifest; current package builds keep upstream tests and Boost.NumPy disabled, and the source treats NumPy as optional test/runtime buffer support.
...
- Keep `openusd-typhoon` as an unconstrained runtime dependency so the default Typhoon renderer works out of the box. GoldenEye and OpenUSD Typhoon may publish to either `test-label` or `default-label`; default-label consumers should prefer the main Anders channel, with the test label available as a fallback during migration, then conda-forge, using channel priority disabled.
