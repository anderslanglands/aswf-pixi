...
- Nightly upstream-release automation should create or refresh a reviewable PR from `automation/upstream-releases`, then dispatch the existing package build workflow on that branch with `publish_target = test-label` by default. Do not automate `default-label` publishes without discussing it with Anders.
