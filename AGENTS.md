...
- Keep native `mitsuba-tools` smoke tests on an explicit LLVM variant such as `llvm_ad_rgb`; scalar rendering is covered through the Python API because upstream Mitsuba 3.8.0 native scalar CLI execution can reach Dr.Jit LLVM state without calling `jit_init(LLVM)` in this bundled build.
