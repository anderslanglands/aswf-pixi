...
- Nightly upstream-release automation should create or refresh a reviewable PR from `automation/upstream-releases`, then dispatch the existing package build workflow on that branch with `publish_target = test-label` by default. When that automation PR is merged to `main`, create a promotion ref at the merge commit and dispatch the existing package build workflow from that ref with `publish_target = default-label` so production uploads still use the `anaconda-production` environment gate. If PR creation is blocked by repository settings, keep the branch/build path non-fatal and surface a manual PR URL; use `UPSTREAM_RELEASE_PR_TOKEN` when a non-`GITHUB_TOKEN` credential is needed for PR creation.
