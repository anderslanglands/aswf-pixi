...
class ChangedRecipeTests(unittest.TestCase):
    ...
    def test_merge_workflow_merges_successful_test_label_automation_builds(self) -> None:
        ...
        self.assertIn("GH_TOKEN: ${{ secrets.UPSTREAM_RELEASE_PR_TOKEN }}", workflow)
        self.assertIn("Require upstream release token", workflow)
        self.assertIn("UPSTREAM_RELEASE_PR_TOKEN is required to merge upstream release PRs", workflow)
