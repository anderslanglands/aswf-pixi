...
class CiMatrixTests(unittest.TestCase):
    ...
    def test_openusd_typhoon_matrix_splits_python_variants(self) -> None:
        recipe = Path("openusd-typhoon/26.05.1.a99d9b6")
        result = ci_matrix.matrix(
            [recipe],
            ["linux-64"],
            {recipe.as_posix(): "2"},
        ...
        self.assertEqual(
            ...
            [
                (
                    "py310",
                    "python=3.10",
                    "openusd-typhoon-26.05.1.a99d9b6-linux-64-py310",
                ),
                (
                    "py311",
                    "python=3.11",
                    "openusd-typhoon-26.05.1.a99d9b6-linux-64-py311",
                ),
                (
                    "py312",
                    "python=3.12",
                    "openusd-typhoon-26.05.1.a99d9b6-linux-64-py312",
                ),
                (
                    "py313",
                    "python=3.13",
                    "openusd-typhoon-26.05.1.a99d9b6-linux-64-py313",
                ),
                (
                    "py314",
                    "python=3.14",
                    "openusd-typhoon-26.05.1.a99d9b6-linux-64-py314",
                ),
            ],
        )
    ...
    def test_openusd_typhoon_root_build_task_uses_relaxed_test_label_channels(self) -> None:
        ...
        self.assertEqual(
            ...
            {"depends-on": ["build-openusd-typhoon-26-05-1-a99d9b6"]},
        )
        build_task = tasks["build-openusd-typhoon-26-05-1-a99d9b6"]
        self.assertIn("--recipe openusd-typhoon/26.05.1.a99d9b6/recipe.yaml", build_task)
    ...
    def test_openusd_typhoon_consumer_manifest_uses_relaxed_test_label_channels(self) -> None:
        recipe_version = "26.05.1.a99d9b6"
    ...
    def test_openusd_typhoon_recipe_is_test_label_only(self) -> None:
        recipe = ROOT / "openusd-typhoon" / "26.05.1.a99d9b6"
        ...
        self.assertEqual(upstream_rev_match.group("rev"), "a99d9b68b5c9c6452a5ef3e237e16f00c3940fea")
    ...
    def test_openusd_recipes_cap_high_parallelism_instead_of_failing(self) -> None:
        cases = [
            ...
            (ROOT / "openusd-typhoon" / "26.05.1.a99d9b6" / "recipe.yaml", 1),
        ]
...
class PublishPackagesTests(unittest.TestCase):
    ...
    def make_typhoon_artifacts(tmp: Path) -> Path:
        ...
        package = artifact_dir / "openusd-typhoon-26.05.1.a99d9b6-py312h123_0.conda"
        package.write_bytes(b"placeholder")
        manifest = tmp / "artifacts" / "job" / "manifest.json"
        manifest.write_text(
            json.dumps(
                {
                    "recipe": "openusd-typhoon/26.05.1.a99d9b6",
                    "platform": "linux-64",
                    "packages": [
                        {
                            "path": "linux-64/openusd-typhoon-26.05.1.a99d9b6-py312h123_0.conda",
                            "file_name": "openusd-typhoon-26.05.1.a99d9b6-py312h123_0.conda",
                            "subdir": "linux-64",
                        }
                    ],
                }
...
""",
    ...
    def test_publish_packages_cli_rejects_default_label_for_real_typhoon_recipe(self) -> None:
        with tempfile.TemporaryDirectory() as tmp_raw:
            ...
            self.assertNotEqual(completed.returncode, 0)
            self.assertIn(
                "Recipe openusd-typhoon/26.05.1.a99d9b6 may only be published to test-label; requested default-label.",
                completed.stderr,
            )
...
class SmokeConsumersTests(unittest.TestCase):
    def test_channel_priority_for_recipe_relaxes_openusd_typhoon_only(self) -> None:
        self.assertEqual(
            smoke_consumers.channel_priority_for_recipe(Path("openusd-typhoon/26.05.1.a99d9b6")),
            "disabled",
        )
    ...
    def test_write_manifest_uses_requested_channel_priority(self) -> None:
        with tempfile.TemporaryDirectory() as tmp_raw:
            ...
            smoke_consumers.write_manifest(
                manifest,
                {"name": "openusd-typhoon", "version": "26.05.1.a99d9b6", "build": "py312h123_0"},
                "linux-64",
                [
                    "https://conda.anaconda.org/anderslanglands/label/test",
                    "https://conda.anaconda.org/anderslanglands",
